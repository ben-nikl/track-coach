...
interface TrackListScreenProps {
    onStartSession?: (track: Track) => void
}
...
const TrackListScreen: React.FC<TrackListScreenProps> = ({onStartSession}) => {
    ...
    const {trackData, sessionActive} = useLapSession();

    // Auto-select active track when returning from lap timer
    useEffect(() => {
        if (sessionActive && trackData && !selectedTrack) {

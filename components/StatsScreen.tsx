...
const StatsScreen: React.FC = () => {
  ...
  if (permission === 'denied') {
    return (
      <View style={styles.center}>
        <Text style={[styles.error, { color: colors.danger }]}>Přístup k GPS zamítnut. Povolit v nastavení.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.wrapper, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 16 }]}>
      ...
      {loading && (
        <View style={styles.row}><ActivityIndicator size="small" /><Text style={[styles.loadingLabel, { color: colors.secondaryText }]}>Načítám…</Text></View>
      )}
      {error && (
        <Text style={[styles.error, { color: error.includes('kCLErrorDomain') ? colors.secondaryText : colors.danger }]}>
      ...
      {coords && (
        <View style={styles.block}>
          ...
          {coords.accuracy != null && <Text style={[styles.meta, { color: colors.secondaryText }]}>± {Math.round(coords.accuracy)} m</Text>}
          {coords.altitude != null && <Text style={[styles.meta, { color: colors.secondaryText }]}>Alt: {Math.round(coords.altitude)} m</Text>}
          <Text style={[styles.meta, { color: colors.secondaryText }]}>{new Date(coords.timestamp).toLocaleTimeString()}</Text>
        </View>
      )}
      <Pressable style={({ pressed }) => [styles.button, { backgroundColor: colors.accent }, pressed && styles.buttonPressed]} onPress={() => fetchLocation()}>
      ...
      {coords && !error && (
        <View style={styles.mapWrapper}>
          ...
          {!mapReady && !mapError && !mapTimeout && (
            <View style={[styles.mapOverlay, { backgroundColor: colors.surface + 'CC' }]}><ActivityIndicator size="small" /><Text style={[styles.mapOverlayText, { color: colors.text }]}>Inicializuji mapu…</Text></View>
          )}
          {mapTimeout && !mapReady && !mapError && (
            <View style={[styles.mapOverlay, { backgroundColor: colors.surface }]}><Text style={[styles.mapOverlayText, { color: colors.text }]}>Mapa se nenačetla. Zkuste restartovat Expo / zkontrolovat react-native-maps.</Text></View>
          )}
          {mapError && (
            <View style={[styles.mapOverlay, { backgroundColor: colors.surface }]}><Text style={[styles.mapOverlayText, { color: colors.danger }]}>Chyba mapy: {mapError}</Text></View>
          )}
        </View>
      )}
      {retryCount >= MAX_RETRIES && error?.includes('kCLErrorDomain error 0') && (
        <Text style={[styles.meta, { color: colors.secondaryText }]}>
          Maximální počet automatických pokusů dosažen. Zkuste ručně nastavit simulátor: Simulator → Features → Location.
        </Text>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  wrapper: { flex: 1, paddingHorizontal: 24, paddingTop: 32 },
  ...
  mapOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', padding: 12 },

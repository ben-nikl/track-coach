...
export const LapSessionProvider: React.FC<{ children: React.ReactNode }> = ({children}) => {
    ...
    }, [trackData, currentLapStartMs, startLap, finishLap]);

    // location setup (foreground + background)
    useEffect(() => {
        ...
        const setup = async () => {
            ...
            console.log('foreground status', fg.status);
            if (fg.status !== 'granted') {
                setPermissionError('Foreground location denied');
                return;
            }
            // Zjistíme, jestli vůbec má smysl background řešit (Expo Go ho často neumí)
            try {
                const isBgAvailable = await Location.isBackgroundLocationAvailableAsync();
                console.log('isBackgroundLocationAvailableAsync:', isBgAvailable);

                if (isBgAvailable) {
                    const bg = await Location.requestBackgroundPermissionsAsync();
                    console.log('background status', bg.status);

                    if (bg.status !== 'granted') {
                        setPermissionError('Background location denied');
                    }
                } else {
                    console.log('Background location not available in this build, skipping BG permission');
                }
            } catch (e) {
                console.log('Error when requesting background permission:', e);
            }
